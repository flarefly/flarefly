...
SGNPDFSDPLUS = ["gaussian", "crystalball", "doublegaus", "doublecb", "doublecbsymm", "genercrystalball"]
BKGPDFSDPLUS = ["expo", "chebpol1"]
SGNPDFSDSTAR = ["gaussian", "voigtian"]
BKGPDFSDSTAR = ["expopow", "powlaw", "expopowext"]
SGNPDFSD0 = ["gaussian", "gausexptail", "genergausexptail", "genergausexptailsymm"]
BKGPDFSD0 = ["expo"]
...
for bkg_pdf in BKGPDFSDPLUS:
    for sgn_pdf in SGNPDFSDPLUS:
        ...
        if sgn_pdf in ["gaussian", "crystalball", "doublecb", "doublecbsymm"]:
        ...
        if sgn_pdf == "doublecbsymm":
            FITTERBINNEDDPLUS[-1].set_signal_initpar(0, "alpha", 2.)
            FITTERBINNEDDPLUS[-1].set_signal_initpar(0, "n", 1.)
        ...
        if bkg_pdf == "chebpol1":
            FITTERBINNEDDPLUS[-1].set_background_initpar(0, "c0", -4.6, limits=[-1.e6, 1.e6])
            FITTERBINNEDDPLUS[-1].set_background_initpar(0, "c1", 1.6, limits=[-1.e6, 1.e6])
        elif bkg_pdf == "expo":
...
for bkg_pdf in BKGPDFSD0:
    for sgn_pdf in SGNPDFSD0:
        ...
        if sgn_pdf in ["gausexptail", "genergausexptailsymm"]:
        ...
        FITTERBINNEDD0[-1].set_background_initpar(0, "c0", 2., limits=[-1.e6, 1.e6])
        FITTERBINNEDD0[-1].set_background_initpar(0, "c1", -0.293708, limits=[-1.e6, 1.e6])
        FITTERBINNEDD0[-1].set_background_initpar(0, "c2", 0.02, limits=[-1.e6, 1.e6])
