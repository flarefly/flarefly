...
FITTER.append(F2MassFitter(DATA, name_signal_pdf=['gaussian'],
                           ...
                           minuit_mode=1,
                           extended=True))
FITTER[1].set_background_initpar(0, 'lam', -10, limits=[-15., -5.], fix=False)
FITRES.append(FITTER[1].mass_zfit(True, prefit_excluded_regions=[1.8, 1.95]))
FIGS.append(FITTER[1].plot_mass_fit(figsize=(10, 10)))
FITTER[1].dump_to_root("test.root", num=100)
...
FITTER.append(F2MassFitter(DATANOBKG2,
                           ...
                           minuit_mode=1,
                           name="nobkg_2signals"))
FITTER[3].set_signal_initpar(0, 'sigma', 0.01, limits=[0., 1.e6])
FITTER[3].set_signal_initpar(1, 'sigma', 0.01, limits=[0., 1.e6])
FITRES.append(FITTER[3].mass_zfit())
...
FITTER.append(F2MassFitter(DATA2, name_signal_pdf=['gaussian', 'gaussian'],
                           ...
                           minuit_mode=1,
                           name="fix_frac"))
FITTER[5].set_background_initpar(0, 'lam', -10, limits=[-15., -5.], fix=False)
FITTER[5].set_particle_mass(0, mass=1.85, limits=[1.84, 1.88])
FITTER[5].set_particle_mass(1, mass=1.95, limits=[1.94, 1.98])
FITTER[5].set_signal_initpar(0, 'sigma', 0.01, limits=[0.005, 0.03])
FITTER[5].set_signal_initpar(1, 'sigma', 0.01, limits=[0.005, 0.03])
FITTER[5].fix_signal_frac_to_signal_pdf(1, 0, 2)
FITRES.append(FITTER[5].mass_zfit(True, prefit_exclude_nsigma=5.))
FIGS.append(FITTER[5].plot_mass_fit(figsize=(10, 10)))
...
FITTER.append(F2MassFitter(DATA2, name_signal_pdf=['gaussian'],
                           ...
                           minuit_mode=1,
                           name="truncated"))
FITTER[6].set_background_initpar(0, 'lam', -10, limits=[-15., -5.], fix=False)
FITTER[6].set_particle_mass(0, mass=1.85, limits=[1.84, 1.88])
FITTER[6].set_signal_initpar(0, 'sigma', 0.01, limits=[0.005, 0.03])
FITRES.append(FITTER[6].mass_zfit())
FIGS.append(FITTER[6].plot_mass_fit(figsize=(10, 10)))
...
def test_fitter_result():
    ...
    for i_fit, fit in enumerate(FITTER):
        ...
        if i_fit in (3, 5):  # test the case with two signals
        ...
        if i_fit not in (2, 3):  # test the cases with background
